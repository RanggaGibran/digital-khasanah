...
function isEmulator() {
    ...
    return process.env.FUNCTIONS_EMULATOR === 'true' ||
        ((_a = process.env.FIREBASE_CONFIG) === null || _a === void 0 ? void 0 : _a.includes('localhost')) ||
...
exports.getUploadUrl = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        throw new functions.https.HttpsError("internal", `Failed to generate upload URL: ${error.message || error}`);
    }
});
...
exports.getDownloadUrl = functions.https.onCall(async (data, context) => {
    ...
    const fileQuery = await firestore
        ...
        .get();
    if (fileQuery.empty) {
        throw new functions.https.HttpsError("not-found", "File not found or you don't have permission to access it.");
    }
    // Path to the file in Cloud Storage
    const filePath = `${userId}/${fileName}`;
    const bucket = (0, storage_1.getStorage)().bucket();
    ...
    if (isEmulator()) {
        ...
        try {
            ...
            throw new functions.https.HttpsError("internal", `Failed to download file: ${error.message}`);
        }
    }
    // Create a signed URL valid for 5 minutes (production mode)
    const [url] = await file.getSignedUrl({
...
exports.deleteFile = functions.https.onCall(async (data, context) => {
    ...
    if (!fileName || !fileId) {
        throw new functions.https.HttpsError("invalid-argument", "File name and ID are required.");
    }
    // Check if this file belongs to the user
    const fileRef = firestore.collection("files").doc(fileId);
    const fileDoc = await fileRef.get();
    if (!fileDoc.exists || ((_a = fileDoc.data()) === null || _a === void 0 ? void 0 : _a.userId) !== userId) {
        throw new functions.https.HttpsError("not-found", "File not found or you don't have permission to delete it.");
    }
    // Path to the file in Cloud Storage
    const filePath = `${userId}/${fileName}`;
    const bucket = (0, storage_1.getStorage)().bucket();
...
exports.saveFileMetadata = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        throw new functions.https.HttpsError("internal", "Failed to save file metadata.");
    }
});
    ...
    corsHandler(req, res, async () => {
        try {
            ...
            try {
                ...
                // Create metadata in Firestore
                try {
                    const fileRef = await firestore.collection("files").add({
                        userId,
                        fileName,
                        fileSize: fileSize || 0,
                        encryptionInfo: encryptionInfo || {},
...
exports.uploadFile = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!fileName || !fileData) {
            throw new functions.https.HttpsError("invalid-argument", "File name and data are required.");
        }
        ...
        throw new functions.https.HttpsError("internal", `Failed to upload file: ${error.message || error}`);
    }
});
